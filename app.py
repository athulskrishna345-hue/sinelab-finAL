...
try:
    ...
    PUSH_ENABLED = False
    print("WARNING: pywebpush/apscheduler not installed.")
...
PROGRESS = {'Handover to Operations':5,'Material Planning':15,'Procurement in Progress':25,
'Material Ready':35,'Material Dispatched':40,'Installation Scheduled':45,
'Installation In Progress':65,'Installation Completed':80,
'Net Metering in Process':88,'Awaiting Approval':90,'Commissioned':95,'Payment Completed':100}
...
def send_push(user_id, title, body, url='/'):
    if not PUSH_ENABLED or not VAPID_PRIVATE or not VAPID_PUBLIC: return
    try:
        conn = get_db()
        r = conn.execute("SELECT subscription FROM push_subscriptions WHERE user_id=?", (user_id,)).fetchone()
        conn.close()
        if not r: return
        webpush(subscription_info=json.loads(r['subscription']),
...
def send_push_to_name(name, title, body, url='/'):
    try:
        ...
        conn.close()
        if u: send_push(u['id'], title, body, url)
    except: pass
...
def check_followups():
    try:
        ...
        conn = get_db()
        leads = conn.execute(
            "SELECT id,name,telecaller FROM leads WHERE follow_up_date=? AND follow_up_time=? "
            "AND status NOT IN ('Order Confirmed','Lost / Dropped')",
            (now.strftime('%Y-%m-%d'), now.strftime('%H:%M'))).fetchall()
        conn.close()
        for l in leads:
...
def login():
    ...
    if not u: conn.close(); return jsonify({'error':'Invalid email or password'}), 401
...
def logout():
    conn = get_db()
    conn.execute("DELETE FROM sessions WHERE token=?", (request.headers.get('X-Token',''),))
    conn.commit(); conn.close()
...
def push_subscribe():
    sub = request.json or {}
    if not sub.get('endpoint'): return jsonify({'error': 'Invalid subscription'}), 400
    conn = get_db()
    conn.execute("INSERT INTO push_subscriptions (user_id, subscription) VALUES (?, ?) ON CONFLICT(user_id) DO UPDATE SET subscription=excluded.subscription",
...
# ── Dashboard — returns exact structure frontend expects ──────
...
def upload_lead_file(lid):
    ...
    if file_type == 'site_photo':
        ...
        if filename not in photos: photos.append(filename)
...
# ── FIX: File serving supports token in header OR query param ─
...
def serve_file(filename):
    ...
    conn = get_db()
    valid = conn.execute(
        "SELECT user_id FROM sessions WHERE token=? AND expires_at>datetime('now')", (token,)).fetchone()
    conn.close()
...
def get_leads():
    ...
    q+=" ORDER BY id DESC"
    conn=get_db(); data=rows(conn.execute(q,p).fetchall()); conn.close()
...
VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (d['name'],d['phone'],d.get('email',''),d.get('location',''),d.get('type','Residential'),
         ...
         20,d.get('status','New Lead'),d.get('survey_date',''),d.get('survey_time',''),
         d.get('survey_eng',''),d.get('lead_date',''),d.get('lead_time','')))
    lid=conn.execute("SELECT last_insert_rowid()").fetchone()[0]; conn.commit()
    lead=row(conn.execute("SELECT * FROM leads WHERE id=?", (lid,)).fetchone()); conn.close()
    log(request.uname, f"Created lead: {d['name']}", 'lead', lid)
    if d.get('telecaller'):
        send_push_to_name(d['telecaller'], '\U0001f31e New Lead Assigned!',
            f"{d['name']} \u2014 {d.get('location','')} \u00b7 {d.get('type','')}", '/#leads')
...
def update_lead(lid):
    d=request.json or {}
    allowed=['name','phone','email','location','type','source','bill','phase','roof','subsidy',
             'kw','status','temp','telecaller','notes','follow_up_date','follow_up_time',
             'follow_up_note','quoted_amount','probability','survey_date','survey_time',
             'survey_eng','lead_date','lead_time']
    conn=get_db()
    old=row(conn.execute("SELECT telecaller FROM leads WHERE id=?", (lid,)).fetchone())
    old_tc=old['telecaller'] if old else ''
    sets=[]; params=[]
    for k in allowed:
    ...
    conn.execute(f"UPDATE leads SET {','.join(sets)} WHERE id=?", params); conn.commit()
    lead=row(conn.execute("SELECT * FROM leads WHERE id=?", (lid,)).fetchone()); conn.close()
    log(request.uname, f"Updated lead: {lead['name']} \u2192 {d.get('status',lead['status'])}", 'lead', lid)
    new_tc=d.get('telecaller','')
    if new_tc and new_tc!=old_tc:
        send_push_to_name(new_tc, '\U0001f31e Lead Assigned to You!',
            f"{lead['name']} \u2014 {lead.get('location','')}", '/#leads')
...
def delete_lead(lid):
    ...
    if not l: return jsonify({'error':'Not found'}), 404
    conn.execute("DELETE FROM leads WHERE id=?", (lid,)); conn.commit(); conn.close()
...
def import_leads():
    ...
    for i,r in enumerate(data):
        name=str(r.get('name','')).strip(); phone=str(r.get('phone','')).strip().replace(' ','')
        if not name or not phone: skipped+=1; errors.append(f"Row {i+2}: missing name/phone"); continue
...
VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'New Lead',?)""",
            (name,phone,r.get('source','Walk-in'),r.get('location',''),r.get('type','Residential'),
             r.get('bill',''),r.get('phase',''),r.get('roof',''),r.get('subsidy','Unknown'),
             r.get('kw',''),r.get('temp','Warm'),r.get('telecaller',''),r.get('notes',''),today))
        imported+=1
    conn.commit(); conn.close()
...
VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (d.get('lead_id'),d['name'],d.get('location',''),d.get('kw',''),
         d.get('status','Handover to Operations'),d.get('progress',5),d.get('payment','Partially Paid'),
         d.get('engineer','Unassigned'),d.get('supervisor',''),d.get('panels',''),
         d.get('inverter',''),d.get('structure',''),d.get('notes',''),datetime.now().strftime('%Y-%m-%d')))
    pid=conn.execute("SELECT last_insert_rowid()").fetchone()[0]; conn.commit()
    proj=row(conn.execute("SELECT * FROM projects WHERE id=?", (pid,)).fetchone()); conn.close()
...
def update_project(pid):
    d=request.json or {}
    allowed=['name','location','kw','status','progress','payment','engineer','supervisor',
             'panels','inverter','structure','notes','net_meter_date','comm_date']
    sets=[]; params=[]
    for k in allowed:
        if k in d: sets.append(f"{k}=?"); params.append(d[k])
    ...
    conn=get_db(); conn.execute(f"UPDATE projects SET {','.join(sets)} WHERE id=?", params); conn.commit()
    proj=row(conn.execute("SELECT * FROM projects WHERE id=?", (pid,)).fetchone()); conn.close()
...
def delete_project(pid):
    if request.urole!='admin': return jsonify({'error':'Admin only'}), 403
    conn=get_db(); conn.execute("DELETE FROM projects WHERE id=?", (pid,)); conn.commit(); conn.close()
...
def create_ticket():
    ...
    tid=next_srv(); tech=d.get('technician','Unassigned')
    ...
    conn.commit()
    ticket=row(conn.execute("SELECT * FROM tickets WHERE id=?", (tid,)).fetchone()); conn.close()
    log(request.uname, f"Ticket {tid}: {d['customer_name']}", 'ticket', tid)
...
def update_ticket(tid):
    ...
    for k in allowed:
        if k in d: sets.append(f"{k}=?"); params.append(d[k])
    if d.get('status')=='Resolved': sets.append('closed_date=?'); params.append(datetime.now().strftime('%Y-%m-%d'))
    if not sets: return jsonify({'error':'Nothing to update'}), 400
    sets.append("updated_at=datetime('now')"); params.append(tid)
    conn=get_db(); conn.execute(f"UPDATE tickets SET {','.join(sets)} WHERE id=?", params); conn.commit()
    ticket=row(conn.execute("SELECT * FROM tickets WHERE id=?", (tid,)).fetchone()); conn.close()
...
def delete_ticket(tid):
    if request.urole!='admin': return jsonify({'error':'Admin only'}), 403
    conn=get_db(); conn.execute("DELETE FROM tickets WHERE id=?", (tid,)); conn.commit(); conn.close()
...
def get_customers():
    conn=get_db()
    data=rows(conn.execute("SELECT c.*,(SELECT COUNT(*) FROM tickets t WHERE t.customer_id=c.id) as ticket_count FROM customers c ORDER BY c.id DESC").fetchall())
    conn.close()
...
def create_customer():
    ...
    if not d.get('name'): return jsonify({'error':'Name required'}), 400
    conn=get_db()
    conn.execute("INSERT INTO customers (project_id,name,location,kw,panels,inverter,comm_date,warranty_end,amc,monitoring_link,inv_login) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
        (d.get('project_id'),d['name'],d.get('location',''),d.get('kw',''),d.get('panels',''),
         d.get('inverter',''),d.get('comm_date',''),d.get('warranty_end',''),
         1 if d.get('amc') else 0,d.get('monitoring_link',''),d.get('inv_login','')))
    cid=conn.execute("SELECT last_insert_rowid()").fetchone()[0]; conn.commit()
    customer=row(conn.execute("SELECT * FROM customers WHERE id=?", (cid,)).fetchone()); conn.close()
...
def get_reports():
    ...
    return jsonify({'by_source':by_src,'by_type':by_type,'by_stage':by_stage,
                    'monthly_leads':monthly,'total_kw':total_kw,'confirmed':confirmed,'total_leads':total_l})
...
def create_user():
    ...
    except: conn.close(); return jsonify({'error':'Email already exists'}), 400
