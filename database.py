...
''')
            ...
            for col in ['survey_date','survey_time','survey_eng','kseb_bill_file',
                        'quotation_file','site_photos','lead_date','lead_time',
                        'follow_up_time','follow_up_note']:
...
def _seed(conn, c):
    ...
    c.executemany(
        "INSERT INTO users (name,email,password_hash,role,department) VALUES (?,?,?,?,?)", users)
